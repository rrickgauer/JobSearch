...
function getPositionsFromQuery($query) {
  ...
  $sql = $pdo->prepare('select Positions.id, Positions.title, Positions.company_id, date_format(Positions.date_applied, "%m-%d-%Y") as date_applied, Positions.date_applied as date_applied_sort, Companies.name as company_name from Positions left join Companies on Positions.company_id=Companies.id WHERE Positions.title like :query group by Positions.id ORDER BY `date_applied_sort` DESC');
...
function getPositionsFromQuerySortByTitle($query) {
  $pdo = dbConnect();

  $sql = $pdo->prepare('select Positions.id, Positions.title, LOWER(Positions.title) as title_sort, Positions.company_id, date_format(Positions.date_applied, "%m-%d-%Y") as date_applied, Positions.date_applied as date_applied_sort, Companies.name as company_name from Positions left join Companies on Positions.company_id=Companies.id WHERE Positions.title like :query group by Positions.id ORDER BY title_sort ASC');
